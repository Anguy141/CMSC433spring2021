...
public class Simulation {
	 ...
	 *                     order
	 */
	public static List<SimulationEvent> runSimulation(int numCustomers, int numCooks, int numTables, int machineCount,
			boolean randomOrders) {

		// This method's signature MUST NOT CHANGE.

		/*
		 * We are providing this events list object for you. It is the ONLY PLACE where
		 * a concurrent collection object is allowed to be used.
		 */
		events = Collections.synchronizedList(new ArrayList<SimulationEvent>());

		// Start the simulation
		logEvent(SimulationEvent.startSimulation(numCustomers, numCooks, numTables, machineCount));

		// Set things up you might need
		new Resturant(numTables, numCustomers);
		Orders.Initialize();

		// Start up machines
		new Kitchen(machineCount);
		...
		try {
			/*
			 * Wait for customers to finish -- you need to add some code here...
			 */
			for (int i = 0; i < customers.length; i++) {
				customers[i].join();
			}
			...
			/*
			 * Then send cooks home... The easiest way to do this might be the following,
			 * where we interrupt their threads. There are other approaches though, so you
	 ...
	 *             in the restaurant, and the fourth is the number of items each set
	 *             of machines can make at the same time.
	 */
